...
<div class="container mb-5">
    <div class="row">
        ...
        <aside class="col-lg-3 mb-4">
            ...
            <!-- Box Lọc Toàn Diện -->
            <div class="card border-0 shadow-sm rounded mb-4 sticky-top custom-scrollbar" style="top: 100px; z-index: 10; max-height: calc(100vh - 120px); overflow-y: auto;">
                ...
                <div class="card-body bg-light">
                    <!-- Form trỏ thẳng về route shop gốc để thực hiện tìm kiếm đa năng -->
                    <form action="{{ route('user.shop') }}" method="GET">
                        ...
                        <!-- DANH MỤC ĐƯỢC GỘP VÀO ĐÂY -->
                        <h6 class="font-weight-bold text-uppercase text-muted mb-2" style="font-size: 13px;">Danh mục sách</h6>
                        <select name="category_id" class="form-control form-control-sm mb-3">
                            <option value="">Tất cả thể loại</option>
                            @foreach($tatCaDanhMuc as $cat)
                                <option value="{{ $cat->id }}" {{ (request('category_id') == $cat->id || (isset($danhMuc) && $danhMuc->id == $cat->id)) ? 'selected' : '' }}>
                                    {{ $cat->name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <button type="submit" class="btn btn-block text-white font-weight-bold shadow-sm" style="background-color: var(--primary-color);">ÁP DỤNG BỘ LỌC</button>
                        
                        @if(request()->anyFilled(['category_id', 'price_min', 'price_max', 'author', 'publisher', 'sort']))
                            <a href="{{ route('user.shop') }}{{ request('keyword') ? '?keyword='.request('keyword') : '' }}" class="btn btn-block btn-outline-secondary mt-2">Xóa bộ lọc</a>
                        @endif
                    </form>
                </div>
            </div>    
        ...
        <section class="col-lg-9">
            ...
            <section class="mt-4 mb-5">
                <style>
                    #heroCarousel{ border-radius:18px; overflow:hidden; box-shadow:0 12px 35px rgba(0,0,0,.12); }
                    #heroCarousel .carousel-item img{ width:100%; height:420px; object-fit:cover; object-position:center; }
                    #heroCarousel .banner-overlay{ position:absolute; inset:0; background:linear-gradient(90deg, rgba(0,0,0,.65) 0%, rgba(0,0,0,.35) 45%, rgba(0,0,0,.08) 100%); }
                    #heroCarousel .banner-content{ position:absolute; top:50%; left:60px; transform:translateY(-50%); max-width:480px; color:#fff; }
                    #heroCarousel .banner-title{ font-size:42px; font-weight:700; line-height:1.25; margin-bottom:15px; text-shadow:0 3px 12px rgba(0,0,0,.35); }
                    #heroCarousel .banner-desc{ font-size:17px; line-height:1.7; color:#f3f3f3; margin-bottom:28px; }
                    #heroCarousel .banner-btn{ padding:12px 30px; border-radius:40px; font-weight:600; transition:.3s; }
                    #heroCarousel .banner-btn:hover{ transform:translateY(-2px); box-shadow:0 8px 20px rgba(255,193,7,.35); }
                    #heroCarousel .carousel-control-prev, #heroCarousel .carousel-control-next{ width:70px; opacity:0; transition:.3s; }
                    #heroCarousel:hover .carousel-control-prev, #heroCarousel:hover .carousel-control-next{ opacity:1; }
                    #heroCarousel .carousel-control-prev-icon, #heroCarousel .carousel-control-next-icon{ width:48px; height:48px; border-radius:50%; background-color:rgba(255,255,255,.22); backdrop-filter:blur(4px); background-size:40%; }
                    #heroCarousel .carousel-indicators{ bottom:15px; margin-bottom:0; }
                    #heroCarousel .carousel-indicators li{ width:8px; height:8px; margin:0 5px; border:none; border-radius:50%; background:#fff; opacity:.45; transition:.3s; }
                    #heroCarousel .carousel-indicators .active{ width:22px; border-radius:20px; background:#ffc107; opacity:1; }
                </style>
            ...
            {{-- ================= TRANG CỬA HÀNG (HIỂN THỊ THEO NHÓM DANH MỤC - KHI KHÔNG CÓ BỘ LỌC) ================= --}}
                ...
                @foreach($sanPhamTheoDanhMuc as $dm)
                    <div class="mb-5 bg-white p-4 rounded shadow-sm border">
                        <div class="d-flex justify-content-between align-items-end mb-4 border-bottom pb-2">
                            ...
                            <a href="{{ route('user.shop', ['category_id' => $dm->id]) }}" class="text-decoration-none" style="color:var(--primary-color);font-weight:600;">
            ...
            {{-- ================= TRANG KẾT QUẢ TÌM KIẾM / ĐÃ ÁP DỤNG BỘ LỌC ================= --}}
            @if(isset($danhSachSanPham))
                <div class="d-flex justify-content-between align-items-center mb-4 border-bottom pb-3">
                    <h2 class="serif-font font-weight-bold mb-0" style="color:#2C3E50;">
                        @if(isset($author) && !request()->has('category_id'))
                            Sách của tác giả {{ $author->name }}
                        @elseif(isset($keyword))
                            Kết quả tìm kiếm: "{{ $keyword }}"
                        @elseif(request()->anyFilled(['category_id', 'price_min', 'price_max', 'publisher', 'author']))
                            Kết quả lọc sản phẩm
                        @elseif(isset($danhMuc))
                            {{ $danhMuc->name }}
                        @else
                            Tất cả sản phẩm
                        @endif
                ...
                @if($danhSachSanPham->isEmpty())
                    <div class="text-center py-5 bg-light rounded shadow-sm border">
                        ...
                        <h5 class="text-muted">Không tìm thấy tác phẩm nào phù hợp với bộ lọc.</h5>
                    </div>
                @else
                    <div class="book-grid">
